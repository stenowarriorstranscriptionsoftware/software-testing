...
// Admin email
const adminEmail = "anishkumar18034@gmail.com";
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    database.ref('users').once('value')
      .then(snapshot => {
        ...
        Object.entries(users).forEach(([uid, userData]) => {
          ...
          userItem.innerHTML = `
            <div class="user-info">
              <img src="${userData.photoURL || 'https://www.gravatar.com/avatar/' + uid + '?d=identicon'}" alt="${userData.displayName}">
              <div>
                <h4>${userData.displayName || 'User'}</h4>
                <p>${userData.email}</p>
                <p class="user-status">Status: ${userData.blocked ? '<span class="danger">Blocked</span>' : '<span class="success">Active</span>'}</p>
              </div>
            </div>
            <button class="${userData.blocked ? 'success-btn' : 'danger-btn'} small-btn" 
                    onclick="toggleUserBlockStatus('${uid}', ${userData.blocked || false})">
  ...
  function toggleUserBlockStatus(userId, currentStatus) {
    const newStatus = !currentStatus;
    database.ref('users/' + userId + '/blocked').set(newStatus)
      .then(() => {
  ...
  function deleteLeaderboardEntry(entryId) {
    ...
    database.ref('attempts/' + entryId).remove()
      .then(() => {
      ...
      <table>
        <thead>
          <tr>
            ...
            ${isAdmin(auth.currentUser) ? '<th>Actions</th>' : ''}
          </tr>
        </thead>
        <tbody>
    `;
      ...
      tableHTML += `
        <tr>
          ...
          ${isAdmin(auth.currentUser) ? 
            `<td><button class="danger-btn small-btn" onclick="deleteLeaderboardEntry('${attempt.id}')">Delete</button></td>` : 
            '<td></td>'}
        </tr>
      `;
    });
        ...
        recentTests.forEach(test => {
          ...
          testCard.innerHTML = `
            <div class="test-header">
              ...
              ${isAdmin(auth.currentUser) ? 
                `<button class="danger-btn small-btn" onclick="deleteTest('${test.id}')">Delete</button>` : 
                ''}
            </div>
